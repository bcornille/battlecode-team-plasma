...
public class Scout {
	...
	/**
	 * run(): Main control method for RobotType Scout
	 * 
	 * @param rc
	...
	static void run(RobotController rc) throws GameActionException {
		...
		while (true) {
			try {
				...
				// Check for enemies, performs movements if engaged, does not
				// update myDirection:
				checkEnemyEngage();
				// Check for units in neutral trees (expensive)
				// checkForUnitTrees();
				// Check for bullets in neutral trees, update direction if
				// necessary:
	...
	/**
	 * checkEnemyEngage(): Determines if there are nearby enemy units and
	 * modifies behavior according to the enemy's RobotType.
	 * 
	 * @throws GameActionException
	 */
	...
	/**
	 * harassClose(): Tells scout to engage in close combat battle with an enemy
	 * unit. This means getting into close proximity and attacking while
	 * following the unit. Attacks are randomly spread a small account to try to
	 * confuse dodge routines and correct for spiral motion.
	 * 
	 * @param target
	 * @throws GameActionException
	 */
	static void harassClose(RobotInfo target) throws GameActionException {
		...
		if (rc.canFireSingleShot()) {
			// fire at target with small spread (helps counter dodging and move
			// error)
	...
	/**
	 * checkForBulletTrees(): A given robot checks for nearby trees that contain
	 * bullets. It checks if there are close trees, and if there are if they
	 * have bullets. If there are no close trees or the close trees do not
	 * contain any bullets, it looks if it can see any trees that do contain
	 * bullets and moves toward the closest one. If there were close trees with
	 * bullets, it sorts all close trees by the number of bullets and shakes the
	 * tree with the most bullets.
	 * 
	 * @param myDirection:
	 *            direction the robot is set to move in
	...
	static Direction checkForBulletTrees(Direction myDirection) throws GameActionException {
		...
		TreeInfo[] closeTrees = rc.senseNearbyTrees(
				RobotPlayer.myType.bodyRadius + GameConstants.INTERACTION_DIST_FROM_EDGE, Team.NEUTRAL);
	...
	/**
	 * checkForUnitTrees(): A given robot checks for nearby trees that contain
	 * robots. If a tree with a robot is found, it checks to see if it has been
	 * found before, and if not, it adds it to the data base of found trees.
	 * 
	 * TODO: Add this to Communications, so all robots can use it TODO: Add
	 * method for setting tree check to false if the tree is no longer there
		...
		if (trees.length == 0) {
			...
			for (TreeInfo tree : trees) {
				...
				if (tree.containedRobot == null) {
					...
					for (int i = TreeConstants.CHANNEL_TREE_UNIT_DATA; i < TreeConstants.CHANNEL_TREE_UNIT_STOP; i += TreeConstants.TREE_UNIT_CHANNELS) {
	...
	/**
	 * processUnitTrees(): Store all of the desired information for a newly
	 * found tree containing a unit, based on the type of unit in the tree. Also
	 * updates the count for the unit-trees of this type.
	 * 
	 * Store: 1) ID of the tree 2) type of robot (as an int: 1 = ARCHON, 2 =
	 * GARDENER, etc) 3) boolean for tree presence (true if there, false if
	 * cleared) 4) x-position of tree 5) y-position of tree
	 * 
	 * @param tree:
	 *            the TreeInfo object being processed
	 * @param channel:
	 *            the communications channel label for the tree
	 * @throws GameActionException
	 */
	static void processUnitTrees(TreeInfo tree, int channel) throws GameActionException {
		...
		case ARCHON:
			...
			rc.broadcast(TreeConstants.CHANNEL_TREE_ARCHON_COUNT,
		...
		case GARDENER:
			...
			rc.broadcastFloat(channel + 3, tree.location.x);
			rc.broadcastFloat(channel + 4, tree.location.y);
			rc.broadcast(TreeConstants.CHANNEL_TREE_GARDENER_COUNT,
		...
		case LUMBERJACK:
			...
			rc.broadcastFloat(channel + 3, tree.location.x);
			rc.broadcastFloat(channel + 4, tree.location.y);
			rc.broadcast(TreeConstants.CHANNEL_TREE_LUMBERJACK_COUNT,
		...
		case SCOUT:
			...
			rc.broadcastFloat(channel + 3, tree.location.x);
			rc.broadcastFloat(channel + 4, tree.location.y);
			rc.broadcast(TreeConstants.CHANNEL_TREE_SCOUT_COUNT,
		...
		case SOLDIER:
			...
			rc.broadcastFloat(channel + 3, tree.location.x);
			rc.broadcastFloat(channel + 4, tree.location.y);
			rc.broadcast(TreeConstants.CHANNEL_TREE_SOLDIER_COUNT,
		...
		case TANK:
			...
			rc.broadcastFloat(channel + 3, tree.location.x);
			rc.broadcastFloat(channel + 4, tree.location.y);
			rc.broadcast(TreeConstants.CHANNEL_TREE_TANK_COUNT,
